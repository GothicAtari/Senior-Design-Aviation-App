    ...
    var body: some View {
        ZStack{
            Image("Group 1")
                ...
                .ignoresSafeArea()
            if let dinner = aviationViewModel.textCorrespondingAvia?.Dinner
            {
                let arrayDinner = sortRestaurants(rests: dinner.components(separatedBy: "; "))
                VStack(spacing: 0){
                    ForEach(arrayDinner, id: \.self) { d in
                        if ((d.isEmpty) != true) {
                            let index = arrayDinner.firstIndex(of: d)
                            VStack(spacing: 0){
                                //Spacer()
                                if (index == 0)
                                {
                    ...
                    if arrayDinner[0] != ""
                    {
                        VStack{
                                ...
                                NavigationLink(
                                    destination: SuggestionPageForSpecificMeals(OneTypeMeal: "Dinner"),
                                    label: {
                                        suggetionButton()
                                    })
    //                        Button(action: {
...
extension Dinner{
    ...
    private var suggestionPage: some View{
        VStack(spacing: 35){
            VStack(spacing: 10) {
                Text("There are no current reviews for dinner for this airport. Please submit your recommendation here:")
                    .font(.subheadline)
                    .fontWeight(.bold)
