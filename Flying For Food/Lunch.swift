    ...
    var body: some View {
        ZStack{
            Image("Group 1")
                ...
                .ignoresSafeArea()
            if let lunch = aviationViewModel.textCorrespondingAvia?.Lunch
            {
                let arrayLunch = sortRestaurants(rests: lunch.components(separatedBy: "; "))
                VStack(spacing: 0){
                    ForEach(arrayLunch, id: \.self) { d in
                        if ((d.isEmpty) != true) {
                            let index = arrayLunch.firstIndex(of: d)
                            VStack(spacing: 0){
                                //Spacer()
                                if (index == 0)
                                {
                    ...
                    if arrayLunch[0] != ""
                    {
                        VStack{
                                ...
                                NavigationLink(
                                    destination: SuggestionPageForSpecificMeals(OneTypeMeal: "Lunch"),
                                    label: {
                                        suggetionButton()
                                    })
    //                        Button(action: {
...
extension Lunch{
    ...
    private var suggestionPage: some View{
        VStack(spacing: 35){
            VStack(spacing: 10) {
                Text("There are no current reviews for lunch for this airport. Please submit your recommendation here:")
                    .font(.subheadline)
                    .fontWeight(.bold)
